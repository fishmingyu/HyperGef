...
# Copyright © 2022 
#
# Modified from AllSet https://github.com/jianhao2016/AllSet
...
from HyperGsys.data.load_dataset import * 
...
class dataset_Hypergraph(InMemoryDataset):
    ...
    def store_graph(self, path):
        ...
        hyperg.store_mtx(path)

    def store_laplacian(self, path):
        data_name = self.name
        data = self.data
        device = 'cuda:0'
        hyperg = HyperGraph(data, device, data_name)
        hyperg.store_laplacian_mtx(path)
...
if __name__ == '__main__':
    ...
    for f in ['walmart-trips', 'house-committees']:
        for feature_noise in [0.1, 1]:
            ...
            print(dd, dd.data)
        dd.store_graph(p2root + "mtx/") # only store noise = 1

    
    p2raw = 'AllSet_all_raw_data/coauthorship/'
    for f in ['coauthor_cora', 'coauthor_dblp']:
        dd = dataset_Hypergraph(root = p2root, 
                name = f,
                p2raw = p2raw)
        dd.store_graph(p2root + "mtx/")
        assert dd.data.num_nodes in dd.data.edge_index[0]
        print(dd, dd.data)

    p2raw = 'AllSet_all_raw_data/cocitation/'
    ...
    p2raw = 'AllSet_all_raw_data/'
    for f in ['NTU2012', 'ModelNet40', 'yelp', 'zoo', '20newsW100', 'Mushroom']:
        dd = dataset_Hypergraph(root = p2root, 
                name = f,
                p2raw = p2raw)
        dd.store_graph(p2root + "mtx/")
